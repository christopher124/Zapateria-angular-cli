...
export class LoginComponent implements OnInit {
  ...
  login() {
    ...
    this.loginServices.login(this.usuario).subscribe(
      (res) => {
        console.log(res);
        //invocar la palabra recervada localStorage
        localStorage.setItem('token', res);
        this.loginServices.erescliente(this.usuario).subscribe(
          (res) => {
          ...
          (err) => {
            console.log(err);
            this.loginServices.eresEmpleado(this.usuario).subscribe(
              (res) => {
